...
export class VectorDatabase {
  ...
  async getStats(): Promise<{
    ...
    try {
      ...
      const subjects = [...new Set(Array.from(this.documents.values()).map(doc => doc.metadata.subject))];
      const books = [...new Set(Array.from(this.documents.values()).map(doc => doc.metadata.bookId))];
