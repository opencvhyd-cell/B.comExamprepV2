...
Now, provide a comprehensive response to the student's question:`;
  ...
  private async getContextualMockResponse(userMessage: string, context?: string[] | { subject?: string; topic?: string }): Promise<AIResponse> {
    ...
    const summaries = this.generateMockSummaries(response, userMessage, subjectContext);
  ...
  private findSubjectContext(context?: any): BComSubjectContext | null {
    if (!context || typeof context !== 'object' || !('subject' in context)) {
      return null;
    }
    ...
    return this.bcomSubjects.find(subject => 
      ...
      subject.subjectCode.toLowerCase().includes(subjectName.toLowerCase())
    ) || null;
  }
...
Would you like me to explain any specific statistical concept or help you practice calculations?`;
  ...
  private getDefaultResponse(question: string, context?: BComSubjectContext | null): string {
    const subjectName = context?.subjectName || 'B.Com subjects';
...
Ask me about any specific topic, and I'll provide detailed explanations with examples and practice problems!`;
  ...
  private generateRelatedQuestions(userMessage: string, context?: any): string[] {
    const lowerMessage = userMessage.toLowerCase();
  ...
  private extractSubjectContext(context?: any): string {
    if (!context || typeof context !== 'object' || !('subject' in context)) {
      return 'General B.Com';
    }
    return context.subject || 'General B.Com';
  }
...
Please provide ONLY the ${summaryType} summary without any additional formatting or labels:`;
  ...
  private async generateSummaries(content: string, userMessage: string, context?: any): Promise<{ long: string; short: string }> {
    try {
      if (this.model) {
        const summaryPrompt = `Based on the following AI response, provide two summaries:
...
1. **Short Summary** (2-3 sentences, maximum 100 words):
   - Key points only
   - Main concepts
   - Essential takeaways

2. **Long Summary** (4-6 sentences, maximum 200 words):
   - Comprehensive overview
   - Detailed explanation
   - Examples and context
   - Study recommendations
...
[Your long summary here]`;
        ...
        return this.parseSummaries(text);
      }
    } catch (error) {
      console.error('Failed to generate summaries with Gemini:', error);
    }
