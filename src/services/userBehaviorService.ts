...
export class UserBehaviorService {
  ...
  private async updateUserAnalytics(
    ...
    try {
      ...
      switch (eventType) {
        case 'test_completed':
          updates.totalTestsTaken = increment(1);
          if (metadata?.score) {
            // Update average score calculation
            const currentAnalytics = await this.getUserAnalytics(userId);
            if (currentAnalytics) {
              const newAverage = (currentAnalytics.averageTestScore * currentAnalytics.totalTestsTaken + metadata.score) / (currentAnalytics.totalTestsTaken + 1);
              updates.averageTestScore = Math.round(newAverage * 100) / 100;
            }
          }
          break;

        case 'study_task_completed':
          updates.totalStudyTime = increment(metadata?.duration || 0);
          break;

        case 'subject_viewed':
          if (metadata?.subject) {
            updates.favoriteSubjects = increment(1);
          }
          break;
