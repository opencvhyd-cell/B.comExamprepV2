...
export default function DashboardStats({ realTimeStats }: DashboardStatsProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      ...
      <StatCard
        title="Tests Taken"
        value={realTimeStats.totalTests.toString()}
        change={`${realTimeStats.completedTests} completed`}
        trend={realTimeStats.totalTests > 0 ? 'up' : 'neutral'}
      ...
      <StatCard
        ...
        value={`${realTimeStats.totalTests > 0 ? Math.round((realTimeStats.completedTests / realTimeStats.totalTests) * 100) : 0}%`}
        change={`${realTimeStats.completedTests}/${realTimeStats.totalTests}`}
        trend={realTimeStats.completedTests > 0 ? 'up' : 'neutral'}
