...
export default function Dashboard() {
  ...
  const { loading: analyticsLoading } = useUserAnalytics(currentUser?.uid || null);
  ...
  const getRealTimeStats = () => {
    ...
    const completedTests = testAttempts?.length || 0; // All test attempts are completed
    ...
    const recentTests = testAttempts?.filter(attempt => 
      new Date(attempt.completedAt) > lastWeek
    ).length || 0;
    ...
    return {
      ...
      recentTests,
      lastTestDate: testAttempts?.[0]?.completedAt || null
    };
  };
