...
export default function StudyAnalytics() {
  ...
  const processAnalytics = (userAnalytics: any, studyPlans: any[]): StudyAnalytics => {
    ...
    subjects.forEach((subject: any) => {
      const subjectTasks = tasks.filter((task: any) => task.subject === subject.name);
      const completedSubjectTasks = subjectTasks.filter((task: any) => task.completed);
      ...
      if (subjectTasks.length > 0) {
        ...
        subjectPerformance[subject.name] = {
          ...
          totalTime: Math.round(subjectTasks.reduce((sum: number, task: any) => sum + (task.duration || 0), 0) / 60 * 100) / 100, // Convert to hours with 2 decimal places
    ...
    Object.entries(subjectPerformance).forEach(([subjectName, performance]: [string, any]) => {
    ...
    // Calculate user-specific optimal study duration based on their plan
    const userOptimalDuration = currentPlan.preferences?.breakDuration 
      ? Math.min(60, Math.max(30, currentPlan.dailyHours * 60 / 4)) // Divide daily hours by 4 for optimal session length
      : 45;

    // Calculate user-specific average focus score
    const userAverageFocusScore = Object.keys(subjectPerformance).length > 0 
      ? Math.round(Object.values(subjectPerformance).reduce((sum: number, perf: any) => sum + perf.averageFocus, 0) / Object.keys(subjectPerformance).length)
      : 70;

    const realAnalytics: StudyAnalytics = {
      totalSessions: totalTasks,
      totalStudyTime: Math.round(tasks.reduce((sum: number, task: any) => sum + (task.duration || 0), 0) / 60), // Convert to hours
