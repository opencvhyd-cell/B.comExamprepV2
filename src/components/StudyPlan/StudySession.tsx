...
export default function StudySession() {
  ...
  const [subjectContent, setSubjectContent] = useState<any>(null);
  ...
  const startSession = () => {
    ...
    // Track session start
    if (currentUser) {
      userBehaviorService.trackEvent(currentUser.uid, 'study_session_started', {
        taskId: task.id,
  ...
  const handleSessionComplete = async () => {
    ...
    try {
      await userBehaviorService.trackEvent(currentUser.uid, 'study_session_completed', {
        taskId: task.id,
        subject: task.subject,
        chapter: task.chapter,
        duration: sessionDuration,
  ...
  const completeTask = async () => {
    if (!task || !currentUser || !studyPlan) return;
    
    try {
      // Mark task as completed
      const updatedTasks = studyPlan.tasks.map((t: StudyTask) =>
        t.id === task.id ? { ...t, completed: true } : t
      );
      
      await studyPlanService.updateStudyPlan(studyPlan.id, { tasks: updatedTasks });
